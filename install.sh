...
then
     ...
     then
          ...
          sudo $kubectl create secret docker-registry regsecret --namespace=$namespace --docker-server=$registry \
     ...
     then
          ...
          sudo helm pull "oci://$registry/tanzu-sql-postgres/vmware-sql-postgres-operator" --version v$operator_version --untar --untardir $unpack_to_dir --plain-http
     ...
     then
          ...
          sudo docker load -i ./images/postgres-instance
          
          echo "LOADING POSTGRES K8S OPERATOR IMAGE..."
          sudo docker load -i ./images/postgres-operator
          ...
          sudo docker tag $(cat ./images/postgres-instance-id) ${INSTANCE_IMAGE_NAME}
          sudo docker push ${INSTANCE_IMAGE_NAME}
          ...
          sudo docker tag $(cat ./images/postgres-operator-id) ${OPERATOR_IMAGE_NAME}
...
sudo ytt -f postgres-crd.yml \
     ...
     | sudo $kubectl --namespace=$namespace apply -f-

sudo $kubectl -n $namespace get postgres $instance_name # -o yaml
